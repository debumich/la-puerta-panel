  ...
  root.innerHTML = `
    <div class="hero card">
      ...
      ${forumStats ? `<div class="hero-stats">
        <div class="hs" data-tone="${forumStats.total > 0 ? 'green' : 'grey'}"><b>${forumStats.total}</b><span>Фракций</span></div>
        <div class="hs" data-tone="${forumStats.soon ? 'amber' : 'green'}"><b>${forumStats.soon}</b><span>Истекает</span></div>
        <div class="hs" data-tone="${forumStats.expired ? 'red' : 'green'}"><b>${forumStats.expired}</b><span>Просрочено</span></div>
        <div class="hs" data-tone="${forumStats.vacant ? 'grey' : 'green'}"><b>${forumStats.vacant}</b><span>Вакантно</span></div>
      </div>` : ''}
    </div>
