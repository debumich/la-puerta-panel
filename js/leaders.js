...
function renderLeaderCard({ key, entry, faction, info }){
  ...
  const appointed = entry.appointedDate || '—';
  ...
  let pointsHtml = '';
  if (isJudicial) {
    const wc = warningsClass(warnings);
    pointsHtml = `<div class="lc-meta-row"><span class="label">Предупреждения</span><span class="value ${wc}">${escapeHtml(warnings)}</span></div>`;
  } else {
    const p = parseInt(points, 10);
    pointsHtml = `<div class="lc-meta-row"><span class="label">Active Points</span><span class="value ${p < 0 ? 'value-negative' : ''}">${escapeHtml(points)}</span></div>`;
  }
  ...
  card.innerHTML = `
    ...
    <div class="lc-meta">
      <div class="lc-meta-row"><span class="label">Назначен</span><span class="value">${escapeHtml(appointed)}</span></div>
      <div class="lc-meta-row"><span class="label">Срок</span><span class="value">${escapeHtml(term)}</span></div>
      ${pointsHtml}
    </div>`;
  return card;
}